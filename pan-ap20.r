...
N <- 1      # Number of tweets per author to average the handcrafter features
TOKEN <- "b9afdf15572e4a46995d16a941a018cd"
path_training <- "C:/Users/David/Desktop/Universidad/Text Mining/pan20-author-profiling-training-2020-02-23"	# Your training path
path_test <- "XXX"			# Your test path
lang <- "es"
...
GenerateVocabulary <- function(path, n = 1000, lowcase = TRUE, punctuations = TRUE, numbers = TRUE, whitespaces = TRUE, swlang = "", swlist = "", verbose = TRUE) {
  ...
  corpus.preprocessed <- tm_map(Vcorpus(VectorSource(corpus.preprocessed)),content_transformer(lemmatize_strings))
...
# GenerateBoW: Given a corpus (training or test), and a vocabulary, obtains the bow representation (de, desde )
GenerateBoW <- function(path, vocabulary, n = 100000, lowcase = TRUE, punctuations = TRUE, numbers = TRUE, whitespaces = TRUE, swlang = "", swlist = "", sentiment_features = FALSE, emotions_features = FALSE, BUA_features = FALSE, verbose = TRUE) {
  setwd(path)
...
# GENERATE VOCABULARY
vocabulary <- GenerateVocabulary(paste(path_training, lang, sep="/"), n, swlang=lang)

# GENDER IDENTIFICATION
#######################
# GENERATING THE BOW FOR THE TRAINING SET
bow_training <- GenerateBoW(paste(path_training, lang, sep="/"), vocabulary, sentiment_features = TRUE, emotion_features = TRUE, BUA_features = TRUE)
...
model_SVM <- train( theclass~., data= training, trControl = train_control, method = "rf")
print(model_SVM)

# Without handcrafted features
training_without <- training[,1:(n+1)]
train_control <- trainControl( method="repeatedcv", number = k , repeats = r)
model_SVM_without <- train( theclass~., data= training_without, trControl = train_control, method = "rf")
